...
## Pre-label validation amendment

The initial smoke test compared every field over all 32 restored outers at 1e-7.
This failed on rho at continuation step six. Investigating before collecting any
training labels: N3 uninterrupted continuations span 687–1043 matvecs; N3 restored
continuations span 690–774, with all six endpoint costs agreeing within 7e-9
relative. Near-stationary late CG is numerically variable even without replay.
The original failed log, driver and registration are retained. Validation now
requires exact restored history, first-four-step cost/lambda/rho/model agreement
at the original 1e-7 tolerance with equal work, and all six 32-step endpoint costs
within 1e-7. Restored total work must fall inside the observed continuous N3 work
range. This amendment affects validation only; training, targets, threshold,
scene split, reward and policy specification are unchanged. No branch-action,
family-check or transfer outcomes were available when making it.
