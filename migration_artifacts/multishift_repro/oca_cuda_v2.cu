...
RunLog SolveMFreeShiftedCG(const DeviceProblem& p, DeviceState& s, Scalar lam0, int max_iter,
  ...
  const bool shared_intr = (CD==9 && p.calib_of_cam!=nullptr && p.ncalib>0);
  ...
  for(int k=0;k<max_iter;){
   if(need_assembly){
    ...
    const bool prune_now = prune_en && accel;
...
static int OcaCliMain(int argc, char** argv) {
  ...
  int tau_persist=0;
  // Early stopping, off by default so the CLI keeps its fixed-iteration
  // behaviour; set both to match the core library (1e-6 / 3).
  double cli_func_tol=0.0;
  int cli_max_consec_fail=0;
  for (int i = 1; i < argc; ++i) {
    ...
    else if (a == "--func-tol") cli_func_tol = std::stod(next());
    else if (a == "--max-consec-fail") cli_max_consec_fail = std::stoi(next());
  ...
  { std::vector<Scalar> f(ncam), k1(ncam), k2(ncam), omega(3*ncam), t(3*ncam);
    ...
    if (algo == "lm") log = SolveLM(p, s, mu0, tol, max_iter, verbose);
      ...
      ? SolveMFreeShiftedCG<9>(
        ...
        (Scalar)cli_func_tol, cli_max_consec_fail)
