#!/usr/bin/env python3
"""One non-ranking Muell diagnostic: shallow CG at every healthy outer."""
import hashlib, json, math, os, pathlib, re, subprocess, sys, time

ROOT=pathlib.Path('/workspace/prism-fast-start/explore-global-cap')
ROOT.mkdir(exist_ok=True)
BIN=pathlib.Path('/workspace/prism-fast-start/build/prism-tr')
DATA=pathlib.Path('/workspace/bal/muell-gba146.txt')
...
stem=ROOT/'muell-global-cap16-1';result=stem.with_suffix('.result.json')
if result.exists():
 print(result.read_text());raise SystemExit
dh,(dims,obs),initial=load_input(DATA,ROOT/'cpu-cache')
flags=json.load(open('/workspace/prism-model-followup/selected_candidate.json'))['flags']
env={k:v for k,v in os.environ.items() if not k.startswith(('OCA_','CASPAR_','COLMAP_MFREE_','MF_DEBUG'))}
env.update(flags,OCA_CKPT_OPEN='16',OCA_TARGET_COST=str(TARGET),OCA_MAX_SECONDS='90')
...
put(stem.with_suffix('.manifest.json'),dict(scope='One development diagnostic only, not an N=3 ranking. OCA_CKPT_OPEN=16 with no window causes the healthy no-reject Muell trajectory to retain the shallow cap.',command=cmd,flags={k:v for k,v in env.items() if k.startswith('OCA_')},target=TARGET,binary_sha256=sha(BIN),source_sha256=sha('/workspace/prism-fast-start/build/source.cu'),data_sha256=dh,initial_cpu_fp64_cost=initial))
t=time.monotonic()
...
if r.returncode==0:
 ...
 assert m and q
 cost=audit(stem.with_suffix('.state'),dims,obs);err_rel=abs(cost-float(m[2]))/max(1,cost);assert math.isfinite(cost) and err_rel<1e-7
...
put(result,row);print(json.dumps(row,indent=2,sort_keys=True))
