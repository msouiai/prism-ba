...
int main(){
 ...
 for(double mask:{0.,1.})for(double cs:{0.,.7,1.})for(double ps:{0.,1.4})for(int pattern:{0,1,2}){
  step=base;for(int i=0;i<9;++i)step[i]*=cs;for(size_t i=9;i<step.size();++i)step[i]*=ps*(pattern==0?1.:(pattern==1?(((i-9)/3)%3==0?0.:1.):(((i-9)/3)%3==0?.125:1.)));
  ...
  for(auto pair:{std::make_pair(g,gpu.slope),std::make_pair(q,gpu.curvature),std::make_pair(-g-.5*q,gpu.prediction)}){double e=std::abs(pair.first-pair.second)/std::max(1.,std::abs(pair.first));maxerr=std::max(maxerr,e);if(e>1e-7)throw std::runtime_error("repaired mixed-step finite-difference mismatch");}++cases;
 }
