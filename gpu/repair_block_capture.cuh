...
inline void PrismCaptureRepairBlocks(const char* prefix,long call,int outer,
    const DeviceProblem& p,const DeviceState& s,const double* step,double cost,
    double full_cost,double camera_cost,double point_cost,double lambda,double tau,int mode,const PrismSubspaceModel::Result& m){
 if(!prefix || !(call==1 || call==2 || call==4))return;
 ...
 std::fprintf(f,"{\"call\":%ld,\"outer\":%d,\"mode\":%d,\"cost\":%.17g,\"full_cost\":%.17g,\"camera_cost\":%.17g,\"point_cost\":%.17g,\"lambda\":%.17g,\"tau\":%.17g,\"gc\":%.17g,\"gp\":%.17g,\"cc\":%.17g,\"cp\":%.17g,\"pp\":%.17g}\n",
   call,outer,mode,cost,full_cost,camera_cost,point_cost,lambda,tau,m.gc,m.gp,m.cc,m.cp,m.pp);
