...
#pragma omp parallel for reduction(+:total) schedule(static) num_threads(TF(nobs_))
  ...
  Result Solve(State* st) {
    ...
    static const bool block_eq = getenv("MF_BLOCKEQ") != nullptr;
    ...
    for (int k = 0; k < o_.max_iterations;) {
      ...
      if (block_eq) { BuildBlockChol(); std::vector<Scalar> tmpb(bprime);
                      ApplyLinv(tmpb.data(), bprime.data()); }
      else for (int i = 0; i < n_c_; ++i) bprime[i] *= E[i];
      ...
      auto Score = [&](const std::vector<Scalar>& x_scaled, int sh, int ck){
        if (block_eq) ApplyLinvT(x_scaled.data(), xc_un.data());
        else for (int i = 0; i < n_c_; ++i) xc_un[i] = x_scaled[i]*E[i];
      ...
      for (cg_it = 0; cg_it < maxck; ++cg_it) {
        if (block_eq) KvB(pv.data(), Ap.data());
        else KvS(pv.data(), Ap.data(), E.data());
      ...
      if (have && best_cost < cost) {
        ...
        if (o_.rho_lambda) {
          ...
          // ===== REVIEW INSTRUMENTATION (MF_PREDAUDIT=1) =====
          // Compare the CG-accumulated pred against the EXACT Gauss-Newton
          // model reduction of the step actually taken, plus the point-only
          // constant 0.5*bp'(V+D)^-1 bp that the reduced (Schur) prediction
          // structurally omits.
          if (getenv("MF_PREDAUDIT")) {
            // exact GN model reduction: -sum r.(J d) - 0.5 sum ||J d||^2
            Scalar t1 = 0.0, t2 = 0.0;
            for (int o2 = 0; o2 < nobs_; ++o2) {
              const int c = p_.camera_index[o2], q = p_.point_index[o2];
              const Scalar* g0 = &gc_[18*(size_t)o2]; const Scalar* g1 = g0 + 9;
              const Scalar* h0 = &gp_[6*(size_t)o2]; const Scalar* h1 = h0 + 3;
              const Scalar* dc = &d_best[9*c];
              const Scalar* dp = &d_best[n_c_ + 3*q];
              Scalar jd0 = 0, jd1 = 0;
              for (int i = 0; i < 9; ++i) { jd0 += g0[i]*dc[i]; jd1 += g1[i]*dc[i]; }
              for (int i = 0; i < 3; ++i) { jd0 += h0[i]*dp[i]; jd1 += h1[i]*dp[i]; }
              t1 += res_[2*(size_t)o2]*jd0 + res_[2*(size_t)o2+1]*jd1;
              t2 += jd0*jd0 + jd1*jd1;
            }
            const Scalar pred_gn = -t1 - 0.5*t2;
            // point-only constant: 0.5 * bp' * Vinv * bp
            std::vector<Scalar> vb(n_p_);
            VinvApply(bp_.data(), vb.data());
            Scalar ptc = 0.0;
            for (int i = 0; i < n_p_; ++i) ptc += 0.5*bp_[i]*vb[i];
            std::printf("  [predaudit] it=%d sh=%d act=%.6e pred_cg=%.6e pred_gn=%.6e "
                        "ptconst=%.6e rho_cg=%.4f rho_gn=%.4f ratio_gn_cg=%.3f\n",
                        k+1, best_sh, (double)act, (double)pred_best, (double)pred_gn,
                        (double)ptc,
                        (double)(pred_best>1e-300? act/pred_best : -1.0),
                        (double)(pred_gn>1e-300? act/pred_gn : -1.0),
                        (double)(pred_best>1e-300? pred_gn/pred_best : -1.0));
          }
          // ===== END INSTRUMENTATION =====
          ...
          if (rho < 0.25) fac = 2.0;
          // REVIEW: MF_FIXDECAY=<f> replaces the whole rho computation with a
          // constant factor, to test whether the trust-region machinery does
          // anything beyond a fixed decay (audit shows rho is inflated by the
          // missing point-only constant, so fac clamps to 1/3 every accept).
          static const Scalar fixdecay = [](){ const char* e=getenv("MF_FIXDECAY");
            return e? (Scalar)atof(e) : (Scalar)0.0; }();
          if (fixdecay > 0.0) fac = fixdecay;
          if (getenv("MF_FACLOG")) std::printf("  [faclog] it=%d rho=%.4f fac=%.4f\n",
                                               k+1,(double)rho,(double)fac);
...
#pragma omp parallel for schedule(static) num_threads(TF(cam_work_))
  ...
  // ===== REVIEW: block-congruence scaling (MF_BLOCKEQ=1) =====
  // Jacobi equilibration scales S by a DIAGONAL D: D S D + sigma I. The same
  // trick works with a per-camera 9x9 factor: build the exact diagonal BLOCK
  // of the Schur complement, S_c = Hcc_c - sum_obs W Vinv W^T (W = gc^T gp),
  // factor S_c = L L^T, and solve L^-1 S L^-T + sigma I. The shift still
  // enters as sigma*I in the transformed space, so multi-shift CG and the
  // zeta recurrence are untouched -- unlike a general preconditioner M, which
  // turns the shift into sigma*M^-1 and destroys the shared Krylov space.
  // Damping semantics become block-Marquardt rather than Levenberg.
  void BuildBlockChol() {
    Lb_.assign(81*(size_t)ncam_, 0.0);
#ifdef _OPENMP
#pragma omp parallel for schedule(static) num_threads(TF(cam_work_))
#endif
    for (int c = 0; c < ncam_; ++c) {
      Scalar B[81];
      const Scalar* H = &Hcc_[81*(size_t)c];
      for (int i = 0; i < 81; ++i) B[i] = H[i];
      for (int idx = cam_off_[c]; idx < cam_off_[c+1]; ++idx) {
        const int o2 = cam_list_[idx]; const int q = p_.point_index[o2];
        if (!ok_[q]) continue;
        const Scalar* g0 = &gc_[18*(size_t)o2]; const Scalar* g1 = g0 + 9;
        const Scalar* h0 = &gp_[6*(size_t)o2]; const Scalar* h1 = h0 + 3;
        Scalar W[27];                       // 9x3, W[i*3+k]
        for (int i = 0; i < 9; ++i) {
          W[i*3+0] = h0[0]*g0[i] + h1[0]*g1[i];
          W[i*3+1] = h0[1]*g0[i] + h1[1]*g1[i];
          W[i*3+2] = h0[2]*g0[i] + h1[2]*g1[i];
        }
        Scalar U[27];
        for (int i = 0; i < 9; ++i) { Scalar u3[3]; Vinv(q, &W[i*3], u3);
          U[i*3+0]=u3[0]; U[i*3+1]=u3[1]; U[i*3+2]=u3[2]; }
        for (int i = 0; i < 9; ++i)
          for (int j = 0; j < 9; ++j)
            B[9*i+j] -= W[i*3+0]*U[j*3+0] + W[i*3+1]*U[j*3+1] + W[i*3+2]*U[j*3+2];
      }
      // symmetrise, floor the diagonal, Cholesky (fall back to diagonal if PD fails)
      Scalar mx = 0, mn = std::numeric_limits<Scalar>::max();
      for (int i = 0; i < 9; ++i) { const Scalar d = std::fabs(B[10*i]);
        mx = std::max(mx, d); mn = std::min(mn, d); }
      // MF_BLOCK_GUARD=<r>: use the block only where it is WELL DETERMINED.
      // Cameras with few observations have near-singular 9x9 Schur blocks; the
      // congruence then amplifies those directions (ladybug-1723: +23% at
      // default tau, +1015% with tau=3e-5). Fall back to diagonal there.
      static const Scalar guard = [](){ const char* e=getenv("MF_BLOCK_GUARD");
        return e? (Scalar)atof(e) : (Scalar)0.0; }();
      // Scale-free conditioning test: Cholesky the NORMALIZED block
      // N = D^-1/2 B D^-1/2 (unit diagonal) and watch the smallest pivot.
      // min/max of the raw diagonal is meaningless here because pose and
      // intrinsics entries carry different units (measured: it flags every
      // camera at any threshold).
      bool weak = false;
      if (guard > 0.0) {
        Scalar N[81], Ln[81] = {0};
        for (int i = 0; i < 9; ++i)
          for (int j = 0; j < 9; ++j) {
            const Scalar di = std::sqrt(std::max(std::fabs(B[10*i]), (Scalar)1e-300));
            const Scalar dj = std::sqrt(std::max(std::fabs(B[10*j]), (Scalar)1e-300));
            N[9*i+j] = 0.5*(B[9*i+j] + B[9*j+i])/(di*dj);
          }
        Scalar minpiv = 1.0;
        for (int i = 0; i < 9 && !weak; ++i)
          for (int j = 0; j <= i; ++j) {
            Scalar sum = N[9*i+j];
            for (int k = 0; k < j; ++k) sum -= Ln[9*i+k]*Ln[9*j+k];
            if (i == j) { if (!(sum > 0)) { weak = true; break; }
                          minpiv = std::min(minpiv, sum); Ln[9*i+j] = std::sqrt(sum); }
            else Ln[9*i+j] = sum / Ln[9*j+j];
          }
        if (!weak && minpiv < guard) weak = true;
      }
      const Scalar fl = std::max(mx*(Scalar)1e-10, (Scalar)1e-30);
      for (int i = 0; i < 9; ++i) { if (!(B[10*i] > fl)) B[10*i] = fl; }
      Scalar* L = &Lb_[81*(size_t)c];
      bool ok = !weak;
      for (int i = 0; i < 9 && ok; ++i) {
        for (int j = 0; j <= i; ++j) {
          Scalar sum = 0.5*(B[9*i+j] + B[9*j+i]);
          for (int k = 0; k < j; ++k) sum -= L[9*i+k]*L[9*j+k];
          if (i == j) { if (!(sum > 0)) { ok = false; break; } L[9*i+j] = std::sqrt(sum); }
          else        L[9*i+j] = sum / L[9*j+j];
        }
      }
      if (!ok) {   // not PD: degrade gracefully to diagonal scaling
        for (int i = 0; i < 81; ++i) L[i] = 0.0;
        for (int i = 0; i < 9; ++i) L[10*i] = std::sqrt(std::max(B[10*i], fl));
      }
    }
  }
  // y = L^-T x   (per camera; L lower)
  void ApplyLinvT(const Scalar* x, Scalar* y) const {
#ifdef _OPENMP
#pragma omp parallel for schedule(static) num_threads(TF(cam_work_))
#endif
    for (int c = 0; c < ncam_; ++c) {
      const Scalar* L = &Lb_[81*(size_t)c];
      const Scalar* xc = x + 9*c; Scalar* yc = y + 9*c;
      for (int i = 8; i >= 0; --i) {
        Scalar s = xc[i];
        for (int k = i+1; k < 9; ++k) s -= L[9*k+i]*yc[k];
        yc[i] = s / L[10*i];
      }
    }
  }
  // y = L^-1 x
  void ApplyLinv(const Scalar* x, Scalar* y) const {
#ifdef _OPENMP
#pragma omp parallel for schedule(static) num_threads(TF(cam_work_))
#endif
    for (int c = 0; c < ncam_; ++c) {
      ...
      for (int i = 0; i < 9; ++i) {
        Scalar s = xc[i];
        for (int k = 0; k < i; ++k) s -= L[9*i+k]*yc[k];
        yc[i] = s / L[10*i];
      }
    }
  }
  void KvB(const Scalar* vin, Scalar* vout) {
    if ((int)scaled_.size() != n_c_) scaled_.resize(n_c_);
    if ((int)scaled2_.size() != n_c_) scaled2_.resize(n_c_);
    ApplyLinvT(vin, scaled_.data());
    Kv(scaled_.data(), scaled2_.data());
    ApplyLinv(scaled2_.data(), vout);
  }
...
#pragma omp parallel for reduction(+:s) schedule(static) num_threads(TF((long)a.size()))
 ...
 private:
  ...
  std::vector<Scalar> tacc2_, uu2_, scaled_, scaled2_, Lb_;
