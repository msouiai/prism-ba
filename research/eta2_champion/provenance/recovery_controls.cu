...
#include "bal_factored_grad.cuh"  // ROUND 10: 12-param GN gradient (pose 6 + f,k1,k2 + point 3)
...
RunLog SolveMFreeShiftedCG(const DeviceProblem& p, DeviceState& s, Scalar lam0, int max_iter,
  ...
  const bool classical_lm=getenv("OCA_CLASSICAL_LM")!=nullptr;
  ...
  PrismRLActor rld; rld.clock=budget_start;
  const int recovery_mode=getenv("OCA_SCHUR_RECOVERY_MODE")?atoi(getenv("OCA_SCHUR_RECOVERY_MODE")):0;
  ...
  const bool camera_tr=getenv("OCA_CAMERA_TR") && atoi(getenv("OCA_CAMERA_TR"))!=0;
  std::unique_ptr<PrismCameraTR> tr;
  if(camera_tr)tr=std::make_unique<PrismCameraTR>(n_c);
  const bool tr_fast_model=getenv("OCA_TR_RECURRENCE")&&atoi(getenv("OCA_TR_RECURRENCE"));
  ...
  if(full_model_rho || camera_tr || classical_lm)full_model=std::make_unique<PrismFullModel>();
  ...
  if((!camera_tr && !classical_lm) || mf_fp32) throw std::runtime_error("mixed storage experiment requires camera TR without --mf-fp32");
  std::printf("MIXED_STORAGE fragments=fp32 arithmetic=fp64 state=fp64 acceptance=fp64\n");
  std::unique_ptr<PrismCameraTR> legacy_tr;
  ...
  if(getenv("OCA_CG_STOP") && atoi(getenv("OCA_CG_STOP"))==2){cg_projection=std::make_unique<PrismCgProjection>(n_c);legacy_tr=std::make_unique<PrismCameraTR>(n_c);}
  using Fragment = float;
  Fragment *Gp=nullptr,*Gc=nullptr,*Bo=nullptr;
  Scalar *Hcc=nullptr,*Cdiag=nullptr,*bc=nullptr,
  ...
  if(compact_fragments){
    ...
    std::printf("  [compact-fragments] active=%d saved_bytes=%zu storage=fp32 camera_reads=%s\n",
                compact_mode,(size_t)nobs*(3*CD*sizeof(Fragment)-sizeof(int)),
  ...
  tr_residual_scales.resize(std::max(1,L),1.);tr_model_shifts.resize(std::max(1,L),0.);
  ...
  if(camera_tr && (CD!=9 || shared_intr || mf_fp32 || rk || block_eq || !use_equil ||
      (L!=1 && L!=5) || demand_mode!=0 || full_model_rho || repair_damping_mode || point_trust_mode ||
      subspace_mode || backtrack_policy || (point_safeguard_mode!=0 && point_safeguard_mode!=1) ||
    ...
    throw std::runtime_error("camera TR requires plain FP64 diagonal one/five-shift mode with point safeguard 1");
  if(classical_lm && (camera_tr || (!attr_rescue&&(point_safeguard_mode || backtrack_on)) || tr_fast_model || full_model_rho || L!=1 || CD!=9 || shared_intr || !use_equil || !pcg))throw std::runtime_error("classical LM requires plain 9DOF single-shift PCG with no TR/safeguard/backtracking");
  if(attr_rescue&&(!backtrack_on||point_safeguard_mode!=1||lm_point_rescue))throw std::runtime_error("matched rescue requires original backtracking and zero/full safeguard, without separate LM rescue");
  ...
  auto RunRIPhase=[&](int nsweeps)->int{
    ...
    for(int it=0; it<ri_open; ++it){
      ...
      else        MFPointFactor<Fragment><<<GridSize(npt),256>>>(Bo,Cdiag,p.point_obs_offsets,p.point_obs_list,ri_tau,npt,Rf,okf);
      CUDA_CHECK(cudaMemset(RId,0,(size_t)n*sizeof(Scalar)));
  ...
  if(replay_save || replay_load){
    ...
    std::vector<std::string> allowed={"OCA_FORCE_UNSHARED","OCA_RHO_LAMBDA",
  ...
  if(replay_load){
    ...
    if(const char* steps=getenv(rld.enabled?"OCA_RLD_STEPS":"OCA_REPLAY_STEPS")) max_iter=replay_start+std::max(1,atoi(steps));
  ...
  for(int k=replay_start;k<max_iter;){
   ...
   if(need_assembly){
    ...
    if(classical_lm&&!attr_split)tau_eff=lam_cam;
    if(camera_tr){
    ...
    const bool prep_fused = CD==9 && use_equil && !e_dead && !rhs_diag_camera;
    if(prep_fused){
      ...
      else        MFRhsDiagCamera<CD,Fragment><<<ncam,256>>>(Gc?Gc:Gp,p.mf_cspt,p.mf_coff,Rf,uu,nobs,corr,diag_out,fragment_slots);
      ...
      else        MFRhsPrime<CD,Fragment><<<GridSize(nobs),256>>>(Gp,fragment_cams,fragment_points,uu,nobs,corr);
    }
    ...
    if(use_equil){
      ...
      if(diag_norm){
        ...
        else        MFDiagK<CD,Fragment,true><<<GridSize(nobs),256>>>(Gp,fragment_points,fragment_cams,Rf,nobs,dk);
      ...
      else        MFDiagK<CD,Fragment><<<GridSize(nobs),256>>>(Gp,fragment_points,fragment_cams,Rf,nobs,dk);
      }
      }
      ...
      if(block_on && !shared_intr){
        ...
        if(block_cm){
          ...
          else        MFBlockSchurCM<CD,Fragment><<<ncam,32>>>(Gc?Gc:Gp,p.mf_cspt,p.mf_coff,Rf,nobs,Bk,fragment_slots);
        } else {
          if(mf_fp32) MFBlockSchur<CD,float><<<GridSize(nobs),256>>>(Gp32,fragment_points,fragment_cams,Rf,nobs,Bk);
          else        MFBlockSchur<CD,Fragment><<<GridSize(nobs),256>>>(Gp,fragment_points,fragment_cams,Rf,nobs,Bk);
        }
    ...
    auto Score=[&](const Scalar* x_scaled,int sh,int ck){
      ...
      if(score_stride>1){
        ...
        else        MFPass1Stride<CD,Fragment><<<GridSize(nobs),256>>>(Gp,fragment_cams,fragment_points,xc_un,nobs,score_stride,tacc);
      } else {
...
#include "tr_recurrence_score.inc"
    ...
    for(int l=0;l<L;++l) shifts[l]=classical_lm?lam_cam:lam_cam*std::pow(10.0,(double)(l-grid_down));
    ...
    std::fill(tr_residual_scales.begin(),tr_residual_scales.end(),1.);
    ...
    for(cg_it=0; cg_it<maxck; ++cg_it){
      ...
      if(cg_projection){double zz=rr;if(pcg)cublasDdot(blas,n_c,pcg->z,1,pcg->z,1,&zz);cg_projection->Append(pcg?pcg->z:r_,Ap_,zz,be_prev,shifts[0]);}
      if(capture_narrow) captured->Append(pv_,Ap_);
      Scalar al=(pcg?pcg->rz:rr)/pAp;
      if(rho_mode) preds[0]+=0.5*al*(pcg?pcg->rz:rr);
      ...
      { const Scalar one=1.0; cublasDaxpy(blas,n_c,&one,pcg?pcg->z:r_,1,pv_,1); }
      rr=rr_new;
      if(prof){cudaDeviceSynchronize();t_mv+=std::chrono::duration<double>(now()-t0).count();}
      while(!classical_lm && ci_<ckpts_eff.size() && cg_it+1==ckpts_eff[ci_]){
...
#include "cg_tr_projection_step.inc"
if(!cg_projection){
#include "tr_cg_stop.inc"
    ...
    if(numeric_guard && trunc && numeric_rebuilds<32 && nc_pp>0 && std::isfinite(nc_pAp)){
    ...
    if(camera_tr){
      ...
      if(legacy_tr && !tr_reuse && legacy_tr->best_prediction>0){
    ...
    int alpha_win=0;
    if(have && use_alpha && (!rho_mode || alpha_rho) && !camera_tr){
    ...
    if(classical_lm && lm_point_rescue && !have){
    ...
    if(have && best_cost<cost){
      ...
      if(classical_lm){
        double f=std::max(1./3.,1.-std::pow(2*lm_rho-1.,3.));lam_cam=attr_radius?attr_next_lambda:std::clamp((double)lam_cam*f,1e-16,1e16);lm_nu=2;
      }
      else if(camera_tr){
        const double anchor=tr->best_sh>=0?shifts[tr->best_sh]:lam_cam;
        // A feasible interior step with an adequate model must not leave
      ...
      if(classical_lm){lam_cam=attr_radius?attr_next_lambda:std::min(1e16,(double)lam_cam*lm_nu);lm_nu=std::min(1e8,lm_nu*2);}
  ...
  if(lm_point_rescue)std::printf("LM_RESCUE summary calls=%ld wins=%ld seconds=%.9g\n",lm_rescue_calls,lm_rescue_wins,lm_rescue_seconds);
  ...
  if(camera_tr)std::printf("CAMERA_TR summary reuses=%ld model_matvecs=%ld full_predictions=%ld radius=%.17g\n",tr->reuses,tr->model_evals,full_model_calls,tr->radius);
  ...
  if(tr_fast_model)std::printf("TR_RECURRENCE_SUMMARY candidates=%ld\n",tr_fast_calls);
  return log;
}
