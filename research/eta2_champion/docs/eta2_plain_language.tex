...
\section{Is Eta2 a trust-region method?}\label{sec:plain-trust-region}

\plainbox{deepblue}{\textbf{Yes, at the level of globalisation; no, if ``trust-region method'' means an exact classical subproblem solve.}
Eta2 owns an explicit camera radius $R$, restricts the proposed camera motion to that radius, measures actual versus predicted improvement with $\rho$, and changes the radius after good or bad agreement.  Those are trust-region ideas.  Its step generator is nevertheless an inexact damped LM solve followed by camera-only clipping and point recomputation.  The most precise name is therefore \emph{radius-controlled inexact LM with trust-region acceptance}.}

\subsection{The textbook reference problem}

At the current state $x_k$, the undamped Gauss--Newton model is
\begin{equation}
  m_k(d)=F(x_k)+g_k^Td+\frac12\lVert J_kd\rVert^2. \label{eq:plain-gn-model}
\end{equation}
A classical scaled trust-region step is defined by one constrained optimization problem:
\begin{equation}
  \min_d\; g_k^Td+\frac12d^T(J_k^TJ_k)d
  \quad\text{subject to}\quad
  \lVert D_k^{1/2}d\rVert\le\Delta_k. \label{eq:plain-classical-tr}
\end{equation}
For an exact solution of this problem, there is a multiplier $\mu\ge0$ satisfying
\begin{align}
  (J_k^TJ_k+\mu D_k)d&=-g_k,\\
  \mu\bigl(\lVert D_k^{1/2}d\rVert-\Delta_k\bigr)&=0. \label{eq:plain-tr-kkt}
\end{align}
In simple terms, the damping $\mu$ is chosen as part of the subproblem so that the resulting step already fits the region.  If the boundary is active, the step lands on it.  It is not first computed somewhere else and then shortened radially.

\subsection{The step Eta2 actually computes}

Eta2 first approximately solves the damped, reduced camera equation
\begin{equation}
  A_\lambda z_\lambda=b_\lambda,
  \qquad
  \lVert b_\lambda-A_\lambda z_\lambda\rVert
  \le \eta\lVert b_\lambda\rVert, \label{eq:plain-inexact-system}
\end{equation}
where the tolerance $\eta$ is often deliberately loose.  It then projects only the scaled camera vector into a ball:
\begin{equation}
  \widehat z
  =z_\lambda\min\!\left(1,\frac{R_k}{\lVert z_\lambda\rVert}\right),
  \qquad \widehat d_c=E\widehat z. \label{eq:plain-implemented-clip}
\end{equation}
The point step is recomputed from $\widehat d_c$ using Eq.~\eqref{eq:plain-backsub}; it is not obtained by uniformly shrinking a previously completed joint step.  The point safeguard may modify that point proposal again.  Call the final camera--point candidate $\widetilde d$.

Eta2 evaluates the original pixel objective and the undamped model on that \emph{actual} candidate:
\begin{align}
  \operatorname{ared}(\widetilde d)
    &=F(x_k)-F(x_k\oplus\widetilde d),\\
  \operatorname{pred}(\widetilde d)
    &=-g_k^T\widetilde d-\frac12\lVert J_k\widetilde d\rVert^2,\\
  \rho_k&=\frac{\operatorname{ared}(\widetilde d)}
                  {\operatorname{pred}(\widetilde d)}. \label{eq:plain-implemented-rho}
\end{align}
It accepts only a finite, decreasing candidate with positive prediction and $\rho_k>0.1$.  Poor agreement contracts $R$; very good boundary agreement may expand it.  The main coupling update is
\begin{equation}
  \lambda_{k+1}=\lambda_k\left(\frac{R_k}{R_{k+1}}\right)^2, \label{eq:plain-lambda-radius}
\end{equation}
although an accepted, well-modelled interior step can reduce $\lambda$ by an additional factor of ten.  Consequently, $\lambda$ is controller state rather than the exact multiplier $\mu$ of Eq.~\eqref{eq:plain-classical-tr}.

\begin{table}[ht]
\centering
\small
\caption{The classical reference problem and the implemented Eta2 method.}
\begin{tabularx}{\linewidth}{@{}p{0.19\linewidth}YY@{}}
\toprule
 & Classical trust-region reference & Prism--Eta2 implementation\\
\midrule
Step definition & Minimize one joint quadratic inside a scaled ball & Approximately solve a damped Schur system, then clip the scaled camera vector\\
Radius & Bounds the chosen norm of the joint step & Bounds $\lVert z\rVert$ for cameras; points are recomputed afterward\\
Damping & Multiplier selected to satisfy the constrained problem & Persistent controller variable, coupled heuristically to $R$\\
Model changes & The returned step solves the stated subproblem & Backtracking and per-track safeguards may change the candidate\\
Acceptance & Usually based on actual/predicted reduction & Based on $\rho$ evaluated on the final, modified candidate\\
\bottomrule
\end{tabularx}
\end{table}

This distinction matters for both theory and novelty.  Standard trust-region convergence theorems do not automatically describe every Eta2 transition, and the project does not claim a new general trust-region theorem.  The research contribution is the measured BA-specific combination and its safeguards, together with evidence about how the inexact linear solve, nonlinear candidate modifications, and numerical curvature recovery interact.
...
\subsection{Where the measurement sits inside CG}

There are two nested loops.  The \emph{outer} BA loop builds a local camera--point model and eventually decides whether to change the cameras and points.  Inside it, the \emph{inner} PCG loop approximately solves the reduced camera equation $A_\lambda z=b_\lambda$.  One BA outer iteration can therefore contain many PCG iterations.

PCG does not jump directly to the final camera vector.  It builds an estimate $z$ by adding one internal search direction $p$ at a time.  A simplified preconditioned-CG iteration is:

\begin{minipage}{\linewidth}
\small
\begin{verbatim}
r = b - A*z                   # unsolved part of the equation
y = ApplyPreconditioner(r)
p = y                         # first internal search direction

repeat:
    w   = ApplyDampedSchur(p)  # w = A*p; the expensive operation
    pAp = dot(p, w)            # denominator required by ordinary CG
    pp  = dot(p, p)

    if not (pAp > 1e-14*pp):  # Eta2's numerical-curvature guard
        raise damping, rebuild, and restart PCG

    alpha = dot(r, y) / pAp    # safe only when pAp is positive
    z     = z + alpha*p
    r_new = r - alpha*w
    y_new = ApplyPreconditioner(r_new)
    beta  = dot(r_new, y_new) / dot(r, y)
    p     = y_new + beta*p
    r, y  = r_new, y_new
end
\end{verbatim}
...
The vector $p$ in this code is \textbf{not yet the BA update}.  It is one direction used to construct the accumulated camera estimate $z$.  It contains scaled camera coordinates only; the point variables have already been eliminated mathematically and are represented implicitly inside \texttt{ApplyDampedSchur}.  The point update is recovered only after PCG finishes and the camera vector has been radius-limited.

The position of the test is crucial: it occurs after computing $A_\lambda p$ and \emph{before} computing \texttt{alpha}.  Ordinary CG already needs $p^TA_\lambda p$ as the denominator of its step length.  If that number is negative, \texttt{alpha} points the wrong way for a convex quadratic.  If it is extremely close to zero, division can create a huge step.  Eta2 turns this already-required denominator into an explicit safety test.

\plainbox{deepblue}{\textbf{What is actually added to CG?}
The expensive Schur product $A_\lambda p$ and the dot product $p^TA_\lambda p$ are required by ordinary PCG.  Eta2 additionally forms $p^Tp$, compares the two numbers, and---only on failure---aborts that inner solve, raises damping, rebuilds the damped system, and restarts.  There is no extra nonlinear cost evaluation and no eigenvalue decomposition.}

\subsection{Why the dot product measures directional curvature}
...
The core of the actual check in the frozen source is therefore:
\begin{verbatim}
...
For a tiny numerical example, take $p=(1,2)^T$ and a scaled quadratic operator $A=\operatorname{diag}(2,0.5)$.  CG obtains $Ap=(2,1)^T$, so
\[
  p^TAp=1\cdot2+2\cdot1=4,
  \qquad p^Tp=1^2+2^2=5,
  \qquad q=0.8.
\]
The positive result says that the quadratic bends upward along this one direction.  Eta2 performs exactly these operations on much larger camera vectors without ever storing $A$.
...
\subsection{Is the curvature guard Eta2's biggest change?}

\textbf{No.}  It is best viewed as a rare robustness mechanism.

\begin{itemize}
  \item \textbf{It did not produce the headline speed result.}  The guard recorded zero repairs before the registered practical targets, including the Final-13682 comparison.  A mechanism that never fired cannot explain those timings.
  \item \textbf{The check itself is standard CG mathematics.}  Positive $p^TAp$ is required by ordinary CG.  The distinctive research result is the captured BA-specific mixed-storage failure, its thin-track diagnosis, and the persistent coupled-damping response---not the invention of the dot product.
  \item \textbf{The main named speed-oriented change is the inexact solve policy.}  Eta2 deliberately permits earlier PCG termination through its doubled adaptive forcing tolerance, then validates the resulting full nonlinear candidate.  The six-scene selection study measured a $1.1633\times$ geometric-mean improvement for the combined global $\lambda_0=0.1$/Eta2 configuration over its incumbent; because the initialization and forcing choice changed together, that number is not a clean one-factor attribution to Eta2 alone.
  \item \textbf{The overall winner is an integration.}  Matrix-free Schur products, the Eta2 forcing rule, camera-radius control, prediction on the actually modified step, point safeguards, and the numerical guard work at different stages.  The evidence does not isolate the curvature recovery as the dominant novelty or speed contribution.
\end{itemize}

The guard can still matter late in a difficult trajectory: without it, one bad denominator can invalidate the PCG recurrence or start a retry loop.  Its supported claim is safer continuation through an observed numerical pathology, rather than faster ordinary iterations.
