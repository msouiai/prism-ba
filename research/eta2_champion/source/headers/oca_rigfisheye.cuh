...
inline RunLog SolveRigFisheye(const RFDeviceProblem& p, RFDeviceState& s,
  ...
  for(int k=0;k<max_iter;){
   if(need_assembly){
    ...
    // REVIEW: OCA_CKPT_MAX=<d> caps the CG sweep (and the scoring ladder) on
    // the RIG path too. On the CD9 path capping at 32 gave -39% wall and
    // -5.3% cost on final-4585; the rig regime is warm and may not behave the
    // same way, which is exactly what this measures. Off by default.
    static const int rf_ckpt_max = [](){ const char* e=getenv("OCA_CKPT_MAX");
      return e? std::atoi(e) : 0; }();
    std::vector<int> ckpts_rf = ckpts;
    if(rf_ckpt_max>0){
      ckpts_rf.clear();
      for(int c : ckpts) if(c<=rf_ckpt_max) ckpts_rf.push_back(c);
      if(ckpts_rf.empty()) ckpts_rf.push_back(rf_ckpt_max);
      maxck = std::min(maxck, rf_ckpt_max);
    }
    ...
    for(cg_it=0; cg_it<maxck; ++cg_it){
      ...
      rr=rr_new;
      while(ci_<ckpts_rf.size() && cg_it+1==ckpts_rf[ci_]){
