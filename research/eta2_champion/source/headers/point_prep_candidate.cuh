#pragma once
// FP64 Gram/Cholesky with an a-posteriori reconstruction check and a
// conservative conditioning screen. The original observation QR is fallback.
__global__ void MFPointFactorGuarded(const float* Bo,const int* poff,const int* plist,int np,double* out,int* fallback){
 ...
 if(!good){for(int j=0;j<6;++j)r[j]=0;for(int k=start;k<end;++k){int o=plist[k];double v[3]={Bo[6ul*o],Bo[6ul*o+1],Bo[6ul*o+2]};MFGivens(r,v);double w[3]={Bo[6ul*o+3],Bo[6ul*o+4],Bo[6ul*o+5]};MFGivens(r,w);}}
 for(int j=0;j<6;++j)out[6ul*p+j]=r[j];if(fallback)fallback[p]=!good;
}
// Same observation-owned FP64 arithmetic as separate RHS/NormOnly diagonal;
// one W load supplies both quantities. Atomics retain their usual order variation.
__global__ void MFRhsDiagFused(const float* W,const int* cam,const int* pt,const double* R,const double* ub,int no,double* corr,double* dk){
 ...
 for(int i=0;i<9;++i){double g[3]={W[(3ul*i)*no+k],W[(3ul*i+1)*no+k],W[(3ul*i+2)*no+k]};double rhs=0;for(int j=0;j<3;++j)rhs+=g[j]*u[j];atomicAdd(corr+9*c+i,rhs);
