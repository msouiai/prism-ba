#!/usr/bin/env python3
"""Registered serial depth-rescue experiment with independent endpoint scoring."""
...
def run(scene,arm,rep,stage,target,input_sha):
    ...
    binary=ORIGINAL if arm=='original' else P/'build/prism-depth'
    ...
    if arm in ['stop','both']:flags['OCA_DEPTH_STOP']='1'
    if arm in ['declip','both']:flags['OCA_DEPTH_DECLIP']='1'
    ...
    write(folder/'manifest.json',dict(command=cmd,flags=flags,binary_sha256=expected,
          source_sha256=bm['parent_source_sha256'] if arm=='original' else bm['source_sha256'],
    ...
    return row
def main():
    ap=argparse.ArgumentParser();ap.add_argument('stage',choices=['compatibility','final','venice','screen'])
    a=ap.parse_args()
    with open('/tmp/prism_gpu.lock','w') as lock:
        ...
        elif a.stage=='final':scenes=['final-3068'];arms=['original','stop'];n=10
        elif a.stage=='venice':scenes=['venice-52'];arms=['original','declip'];n=10
        else:scenes=['dubrovnik-88','ladybug-1197'];arms=['original','off','stop','declip','both'];n=3
        rows=[]
        for s in scenes:
            for rep in range(n):
                j=rep%len(arms)
