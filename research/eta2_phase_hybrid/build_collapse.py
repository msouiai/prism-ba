...
def main():
  ...
  const int hybrid_open_limit=getenv("OCA_PHASE_OPEN_LIMIT")?atoi(getenv("OCA_PHASE_OPEN_LIMIT")):8;
  if(hybrid_open_limit<1||hybrid_open_limit>600)throw std::runtime_error("phase open limit must be 1..600");
...
''')
    ...
    rep('''    if(TargetReached(cost,k+1)) break;''','''    if(hybrid_open){
      ...
      if(hybrid_flat_streak>=2||n_accept>=hybrid_open_limit){
        hybrid_open=false;
  ...
  if(numeric_guard)std::printf("NUMERIC_REPAIR summary''')
    b=P/'build_collapse';b.mkdir(exist_ok=True);out=b/'prism-hybrid';s=b/'prism_hybrid.cu';s.write_text(code)
    ...
    (P/'collapse_build_manifest.json').write_text(json.dumps(dict(command=cmd,binary_sha256=sha(out),source_sha256=sha(s),base_source_sha256=sha(src),sweep_header_sha256=sha(P/'shift_sweep.cuh')),indent=2)+'\n');print(out,flush=True)
