...
def main():
    ...
    build=P/'build_conditional';build.mkdir(exist_ok=True)
    code=src.read_text()
    def replace(old,new):
        nonlocal code
  ...
  const bool follow_gate=getenv("OCA_FOLLOW_GATE")&&atoi(getenv("OCA_FOLLOW_GATE"));
...
''')
    ...
    replace('''      auto model=full_model->Evaluate(p,s,have?d_best:dfull,k2mask);++full_model_calls;
      ...
      const double follow_original_rho=have&&model.prediction>0?(cost-best_cost)/model.prediction:-1;
      if(follow_point && ((have && best_cost<cost) || follow_gate) &&
         (!follow_gate || !std::isfinite(follow_original_rho) || follow_original_rho<=.25)){
        const auto begin=now();++follow_calls;
        if(have)CUDA_CHECK(cudaMemcpy(dfull,d_best,(size_t)n*sizeof(Scalar),cudaMemcpyDeviceToDevice));
        ...
        if(changed){
          ...
          if(std::isfinite(candidate)&&candidate<(have?best_cost:cost)){
            ...
            if(std::isfinite(rho)&&rho>.1){
              ...
              if(!have){best_sh=0;best_ck=cg_broke?cg_it+1:maxck;have=true;}
            }
          }
        }
  ...
  if(selective && !PrismLowParallax(offsets,order,ci,R,t,X,point,lane))return;''')
    ...
    (P/'conditional_build_manifest.json').write_text(json.dumps({'command':cmd,'binary_sha256':sha(build/'prism-followup'),'source_sha256':sha(source),'base_source_sha256':sha(src),'overlay_sha256':sha(P/'point_overlay.cuh')},indent=2)+'\n')
