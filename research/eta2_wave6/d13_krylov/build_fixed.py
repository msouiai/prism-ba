#!/usr/bin/env python3
"""Build the checksum-pinned fixed-system restart screen."""
...
OUT=pathlib.Path('/tmp/prism-wave6-d13-build');OUT.mkdir(parents=True,exist_ok=True)
...
assert all(hashlib.sha256((PKG/'source/headers'/name).read_bytes()).hexdigest()==digest for name,digest in manifest['headers_sha256'].items())
parts=[]
for start,end in [
...
binary=OUT/'restart_fixed'
command=['nvcc','-O3','-DNDEBUG','-std=c++17','-arch=sm_89','-I/usr/include/eigen3',f'-I{OUT}',str(HERE/'restart_fixed.cu'),'-o',str(binary),'-lcublas']
with (OUT/'build.log').open('w') as log:subprocess.run(command,stdout=log,stderr=subprocess.STDOUT,check=True)
record={'command':command,'source_sha256':hashlib.sha256((HERE/'restart_fixed.cu').read_bytes()).hexdigest(),
        'reference_sha256':hashlib.sha256((OUT/'reference.cuh').read_bytes()).hexdigest(),
        'binary_sha256':hashlib.sha256(binary.read_bytes()).hexdigest()}
(HERE/'evidence').mkdir(exist_ok=True);(HERE/'evidence/build_manifest.json').write_text(json.dumps(record,indent=2)+'\n')
