#!/usr/bin/env python3
"""Build D16 from the exact D0v3 deterministic source."""
from __future__ import annotations
import hashlib,importlib.util,json,os,pathlib,subprocess

HERE=pathlib.Path(__file__).resolve().parent;W6=HERE.parent;F=W6.parent/'eta2_champion';OUT=pathlib.Path('/tmp/prism-wave6-d16');OUT.mkdir(parents=True,exist_ok=True)
...
patch('''  std::unique_ptr<PrismPcg> pcg;
  ...
  std::unique_ptr<D16CountProject> d16;
  if(getenv("OCA_D16_COUNT_PROJECT"))d16=std::make_unique<D16CountProject>(ncam,p.mf_coff,p.mf_cspt);''')
patch('''      if(attr_radius){
        ...
        if(!(attr_R>0))attr_R=std::isfinite(attr_raw_norm)&&attr_raw_norm>0?attr_raw_norm:1;
        if(d16){
          auto q=d16->Inspect(xs[0],attr_R);const bool trigger=q.ratio>100&&q.gated_fraction>.5;double post=q.norm;
          if(trigger){d16->Project(xs[0]);PrismW6Dnrm2(blas,n_c,xs[0],1,&post);attr_raw_norm=post;}
          std::printf("D16_TEST o=%d retry=%d ratio=%.17g gated_fraction=%.17g raw=%.17g radius=%.17g trigger=%d post_raw=%.17g post_ratio=%.17g\\n",k,retries,q.ratio,q.gated_fraction,q.norm,attr_R,(int)trigger,post,post/attr_R);
        }
        attr_old_R=attr_R;
        if(attr_raw_norm>attr_R){double scale=attr_R/attr_raw_norm;cublasDscal(blas,n_c,&scale,xs[0],1);}
      }''')
patch('  if(final_lambda_out) *final_lambda_out = lam_cam;','  if(d16)d16->Final();\n  if(final_lambda_out) *final_lambda_out = lam_cam;')
restored=s
for a,b in reversed(patches):assert restored.count(b)==1;restored=restored.replace(b,a)
assert restored==source
src=OUT/'prism_d16.cu';binary=OUT/'prism-d16';src.write_text(s)
...
record={'command':cmd,'parent_source_sha256':hashlib.sha256(source.encode()).hexdigest(),'derived_source_sha256':sha(src),'binary_sha256':sha(binary),'inherited_patch_count':inherited,'d16_patch_count':len(patches),'sources':{str(p):sha(p) for p in [HERE/'build.py',HERE/'d16_count_project.cuh',W6/'build_deterministic.py']},'protocol_sha256':sha(W6/'D16_COUNT_PROJECT_PROTOCOL.md')}
