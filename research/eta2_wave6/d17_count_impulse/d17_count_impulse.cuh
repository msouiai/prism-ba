...
__global__ void D17ProjectKernel(int nc,const int*gate,double*x){
  int c=blockIdx.x*blockDim.x+threadIdx.x;
  if(c<nc&&gate[c])for(int j=0;j<8;++j)x[9*c+j]=0.;
}

struct D17CountImpulse{
  int nc,no;int*d_gate=nullptr;std::vector<int>offsets,points,counts,gate;
  std::vector<double>host_step;long tests=0,impulses=0;bool spent=false;
  D17CountImpulse(int n,const int*d_off,const int*d_points):nc(n){
    ...
    std::printf("D17_GATE ncam=%d median=%.17g selected=%d ids=",nc,med,std::accumulate(gate.begin(),gate.end(),0));for(int c=0;c<nc;++c)if(gate[c])std::printf("%d:%d,",c,counts[c]);std::printf("\n");
  }
  ~D17CountImpulse(){cudaFree(d_gate);}
  ...
  void Project(double*z){D17ProjectKernel<<<(nc+63)/64,64>>>(nc,d_gate,z);spent=true;++impulses;}
  void Final(){std::printf("D17_SUMMARY tests=%ld impulses=%ld spent=%d\n",tests,impulses,(int)spent);}
};
