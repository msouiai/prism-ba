...
| Exact per-track algebra | Wave-5 A1 repairs the E4 witness almost exactly, but native Final3068 is unchanged and Venice worsens 4.15%. D22 then applies it only once at a proposed stop: 2/5 hits in both arms and just 0.001304% median terminal decrease across misses. | Closed as both an always-on policy and a fixed-camera terminal audit. |
...
| Early restart predictor | D24 fits five fixed outer-15 scalar rules on 80 B6v7 runs and validates on the next 50. Every train/validation balanced accuracy is below 0.62; the high-sensitivity cost rule falsely aborts 25/32 validation hits. | Fixed interpretable predictor closed without opening the sealed D21 test set. |
