#!/usr/bin/env python3
"""Deterministic common-random-number confirmation for D15."""
...
BINARY = pathlib.Path("/tmp/prism-wave6-d15-deterministic/prism-d15-deterministic")
BUILD = HERE / "deterministic-build-manifest.json"
PROTOCOL = WAVE6 / "D15_PAIRED_CONFIRMATION_PROTOCOL.md"
SOURCE = pathlib.Path("/workspace/bal/final-3068.txt")
TARGET = 1744796.9841897595
SEEDS = list(range(660000, 660024))
EPSILON = 1e-12
...
def registration() -> dict:
    ...
    reg = {
        ...
        "runner_sha256": sha256(HERE / "run_paired.py"),
        ...
        "sprt": {"q0": .50, "q1": .70, "alpha": .05, "beta": .05,
                 "max_pairs": len(SEEDS)},
        "arm_order": "control,d15 on even pair index; d15,control on odd pair index",
    }
    N.write(WAVE6 / "d15-paired-registration.json", reg)
    return reg
...
def trace_fields(row: dict) -> dict:
    ...
    for line in (folder / "stdout.log").read_text().splitlines():
        ...
        if line.startswith(prefixes):
            lines.append(line.replace(" d15_prior=0", ""))
    ...
    tests = list(re.finditer(
        ...
        r"raw=(\S+) radius=(\S+) trigger=(\d+)",
        (folder / "stdout.log").read_text(),
    ))
    summary = re.search(r"D15_SUMMARY tests=(\d+) activations=(\d+) build_seconds=(\S+)",
                        (folder / "stdout.log").read_text())
    row["d15"] = {
        "tests": int(summary[1]) if summary else 0,
        "activations": int(summary[2]) if summary else 0,
        "build_seconds": float(summary[3]) if summary else 0.0,
...
def compatibility(reg: dict) -> dict:
    ...
    rows = []
    for arm, binary in [("d0v3-parent", PARENT), ("d15-derived-off", BINARY)]:
        folder = WAVE6 / "evidence" / "d15-paired-compatibility" / arm
    ...
    N.write(WAVE6 / "d15-paired-compatibility.json", result)
...
def run_arm(reg: dict, cell: dict, seed: int, index: int, arm: str) -> dict:
    folder = WAVE6 / "evidence" / "d15-paired-final3068" / f"{seed}-{arm}"
    flags = dict(FLAGS)
    if arm == "d15":
        flags["OCA_D15_COUNT_PRIOR"] = "1"
...
def summarize(pairs: list[dict], reg: dict) -> dict:
    ...
    for pair in pairs:
        if s.decision != "continue":
            break
        s.update(pair["control"]["hit"], pair["d15"]["hit"])
    double = [p for p in pairs if p["control"]["hit"] and p["d15"]["hit"]]
    endpoint = [p["d15"]["cost"] / p["control"]["cost"] - 1 for p in pairs]
    time_ratios = [p["d15"]["target_seconds"] / p["control"]["target_seconds"]
                   for p in double]
    report = s.report()
    report.update({
        "completed_pairs": len(pairs),
        "control_hits": sum(p["control"]["hit"] for p in pairs),
        "d15_hits": sum(p["d15"]["hit"] for p in pairs),
        ...
        "d15_activated_pairs": sum(p["d15"]["d15"]["activations"] > 0 for p in pairs),
        "d15_total_activations": sum(p["d15"]["d15"]["activations"] for p in pairs),
        "promotion_time_gate": (
    ...
    report["decision_interpretation"] = {
        "noninferior": "D15-better boundary",
        "harmful": "D15-harmful boundary",
...
def execute(reg: dict) -> dict:
    assert json.loads((WAVE6 / "d15-paired-compatibility.json").read_text())["passed"]
    problem = load_bal(SOURCE)
    initial = state_from_bal(problem)
    pairs = []
    result_path = WAVE6 / "d15-paired-results.json"
    ...
    for index, seed in enumerate(SEEDS):
        ...
        path = pathlib.Path(f"/dev/shm/eta2-d15-paired-{seed}.txt")
        ...
        arms = ["control", "d15"] if index % 2 == 0 else ["d15", "control"]
        rows = {}
        try:
            for arm in arms:
        ...
        pair = {"index": index, "seed": seed, "epsilon": EPSILON,
                ...
                "control": rows["control"], "d15": rows["d15"]}
        pairs.append(pair)
        N.write(result_path, pairs)
        report = summarize(pairs, reg)
        N.write(WAVE6 / "d15-paired-summary.json", report)
        print("PAIR", index + 1, "seed", seed,
              "hits", int(pair["control"]["hit"]), int(pair["d15"]["hit"]),
              "activations", pair["d15"]["d15"]["activations"],
...
def main() -> None:
    ...
    if args.stage == "register":
        ...
        result = summarize(json.loads((WAVE6 / "d15-paired-results.json").read_text()), reg)
        N.write(WAVE6 / "d15-paired-summary.json", result)
