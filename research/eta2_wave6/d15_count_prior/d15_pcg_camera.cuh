...
__global__ void D15PcgAdd(int nc,const double*D,double*B){int c=blockIdx.x*blockDim.x+threadIdx.x;if(c<nc)for(int i=0;i<9;++i)for(int j=0;j<9;++j)B[81ul*c+9*i+j]+=D[81ul*c+9*i+j];}
...
struct PrismPcg{
 int nc,n,last_outer=-1,last_depth=0;double*B,*z,*tmp,*diag;int*bad;double rz=0;bool reuse,schur;const double*prior=nullptr;
 ...
 void SetPrior(const double*p){prior=p;}
 template<class F>
 void Prepare(const double*H,const double*E,double sig,int outer,int retry,const F*W,const int*pt,const int*off,const double*R,int no){bool rebuild=prior||!reuse||last_outer<0||retry||last_depth>64;int drift=0;if(!rebuild){CUDA_CHECK(cudaMemset(bad,0,4));PcgDiagDrift<<<(nc+255)/256,256>>>(nc,H,E,sig,diag,bad);CUDA_CHECK(cudaMemcpy(&drift,bad,4,cudaMemcpyDeviceToHost));rebuild=drift;}
 if(rebuild){if(schur){CUDA_CHECK(cudaMemset(B,0,81ul*nc*8));MFBlockSchurCM<9,F><<<nc,32>>>(W,pt,off,R,no,B);MFBlockAddHcc<9><<<(nc+255)/256,256>>>(H,nc,B);}PcgNormalize<<<(nc+255)/256,256>>>(nc,schur?B:H,E,sig,B);if(prior)D15PcgAdd<<<(nc+255)/256,256>>>(nc,prior,B);PcgReferenceDiag<<<(nc+255)/256,256>>>(nc,H,E,sig,diag);MFBlockChol<9><<<(nc+255)/256,256>>>(B,nc,1e-10,nullptr);last_outer=outer;}
 printf("PCG_PREP o=%d rebuild=%d drift=%d previous_depth=%d d15_prior=%d\n",outer,(int)rebuild,drift,last_depth,(int)(prior!=nullptr));
 }
