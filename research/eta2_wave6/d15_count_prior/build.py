#!/usr/bin/env python3
"""Build checksum-pinned D15 fixed-system sparse-prior harness."""
...
OUT = pathlib.Path("/tmp/prism-wave6-d15-build")
...
binary = OUT / "count_prior_fixed"
command = [
    "nvcc", "-O3", "-DNDEBUG", "-std=c++17", "-arch=sm_89",
    "-I/usr/include/eigen3", f"-I{OUT}", str(HERE / "count_prior_fixed.cu"),
    "-o", str(binary), "-lcublas",
]
with (OUT / "build.log").open("w") as log:
    subprocess.run(command, stdout=log, stderr=subprocess.STDOUT, check=True,
                   env={**__import__("os").environ, "TMPDIR": "/dev/shm"})
record = {
    "command": command,
    "frozen_source_sha256": hashlib.sha256(source.encode()).hexdigest(),
    "reference_sha256": sha(OUT / "reference.cuh"),
    "source_sha256": sha(HERE / "count_prior_fixed.cu"),
    "binary_sha256": sha(binary),
    "protocol_sha256": sha(ROOT / "research/eta2_wave6/D15_COUNT_PRIOR_PROTOCOL.md"),
}
