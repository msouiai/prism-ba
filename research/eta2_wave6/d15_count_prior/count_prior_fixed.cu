...
static std::vector<int> unique_track_counts(int nc,const std::vector<int>&cams,const std::vector<int>&pts){
  std::vector<unsigned long long> pairs(cams.size());for(size_t i=0;i<cams.size();++i)pairs[i]=(unsigned long long)(unsigned)cams[i]<<32|(unsigned)pts[i];
  std::sort(pairs.begin(),pairs.end());pairs.erase(std::unique(pairs.begin(),pairs.end()),pairs.end());std::vector<int> counts(nc);
  for(auto key:pairs)++counts[(int)(key>>32)];return counts;
}

struct PriorGeometry{std::vector<Eigen::Matrix<double,8,8>> blocks;std::vector<int> counts,gate;double median_count=0,mu_ref=0;};
static PriorGeometry prior_geometry(int nc,long long no,const std::vector<int>&ch,const std::vector<int>&ph,const std::vector<double>&E,DeviceSystem&s){
  ...
  PriorGeometry out;out.counts=unique_track_counts(nc,ch,ph);out.gate.assign(nc,0);out.blocks.resize(nc);
  auto sorted_counts=out.counts;std::sort(sorted_counts.begin(),sorted_counts.end());out.median_count=sorted_counts[nc/2];if(nc%2==0)out.median_count=.5*(out.median_count+sorted_counts[nc/2-1]);
  std::vector<int> order(nc);std::iota(order.begin(),order.end(),0);std::sort(order.begin(),order.end(),[&](int a,int b){return std::pair<int,int>(out.counts[a],a)<std::pair<int,int>(out.counts[b],b);});
  int q=(nc+99)/100;for(int k=0;k<q;++k)if(out.counts[order[k]]<out.median_count/4)out.gate[order[k]]=1;
  std::vector<double> maxima;maxima.reserve(nc);
  for(int c=0;c<nc;++c){Eigen::Matrix<double,8,8>A;for(int i=0;i<8;++i)for(int j=0;j<8;++j)A(i,j)=.5*(B[(size_t)81*c+9*i+j]+B[(size_t)81*c+9*j+i])*E[9*c+i]*E[9*c+j];out.blocks[c]=A;
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,8,8>> es(A);if(es.info()!=Eigen::Success)throw std::runtime_error("block eigensolve");if(!out.gate[c])maxima.push_back(es.eigenvalues()[7]);}
  std::sort(maxima.begin(),maxima.end());out.mu_ref=maxima[maxima.size()/2];if(maxima.size()%2==0)out.mu_ref=.5*(out.mu_ref+maxima[maxima.size()/2-1]);return out;
}
...
int main(int argc,char**argv){try{
  ...
  auto W=load_host<float>(dir+"W",(size_t)27*no);auto U=load_host<double>(dir+"U",(size_t)81*nc);auto R=load_host<double>(dir+"R",(size_t)6*np);auto E=load_host<double>(dir+"E",(size_t)9*nc);auto b=load_host<double>(dir+"b",(size_t)9*nc);auto cams=load_host<int>(dir+"cams",no);auto pts=load_host<int>(dir+"points",no);auto offs=load_host<int>(dir+"offsets",nc+1);
  DeviceSystem s(nc,np,no,sigma,W,U,R,E,b,cams,pts,offs);auto g=prior_geometry(nc,no,cams,pts,E,s);int ng=std::accumulate(g.gate.begin(),g.gate.end(),0);
  std::printf("SYSTEM nc=%d np=%d no=%d sigma=%.17g eta=%.17g median_count=%.17g gated=%d mu_ref=%.17g ids=",nc,np,no,sigma,eta,g.median_count,ng,g.mu_ref);for(int c=0;c<nc;++c)if(g.gate[c])std::printf("%d:%d,",c,g.counts[c]);std::printf("\n");
  struct Arm{const char*name;double dose;bool constraint;};std::vector<Arm>arms={{"0",0,false},{"1",1,false},{"10",10,false},{"100",100,false},{"inf",0,true}};
