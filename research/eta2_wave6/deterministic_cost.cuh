...
inline Scalar PrismW6DeterministicCost(const DeviceProblem& p,
  ...
  W6CostPartials<<<blocks, 256>>>(
      ...
      INTR_F(p,s), INTR_K1(p,s), INTR_K2(p,s), p.nobs, partials, rk, rk_a2);
  W6ReduceScalarFixed<<<1,256>>>(partials, blocks, output);
  Scalar cost = 0.0;
  CUDA_CHECK(cudaMemcpy(&cost, output, sizeof(Scalar), cudaMemcpyDeviceToHost));
  return cost;
}
