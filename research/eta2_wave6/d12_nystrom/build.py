#!/usr/bin/env python3
"""Build the checksum-pinned fixed-system Nyström screen."""
...
OUT=pathlib.Path('/tmp/prism-wave6-d12-build');OUT.mkdir(parents=True,exist_ok=True)
...
binary=OUT/'nystrom_fixed'
command=['nvcc','-O3','-DNDEBUG','-std=c++17','-arch=sm_89','-I/usr/include/eigen3',f'-I{OUT}',str(HERE/'nystrom_fixed.cu'),'-o',str(binary),'-lcublas']
with (OUT/'build.log').open('w') as log:subprocess.run(command,stdout=log,stderr=subprocess.STDOUT,check=True)
record={'command':command,'source_sha256':hashlib.sha256((HERE/'nystrom_fixed.cu').read_bytes()).hexdigest(),
