...
static std::vector<double> build_delta(const PriorGeometry&g,double dose,bool constraint){
  ...
  for(int c=0;c<nc;++c)if(g.gate[c]){Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,8,8>> es(g.blocks[c]);if(es.info()!=Eigen::Success)throw std::runtime_error("prior eigensolve");
    Eigen::Array<double,8,1>d=Eigen::Array<double,8,1>::Zero();d[0]=std::max(0.0,dose*g.mu_ref-es.eigenvalues()[0]);Eigen::Matrix<double,8,8>D=es.eigenvectors()*d.matrix().asDiagonal()*es.eigenvectors().transpose();
...
int main(int argc,char**argv){try{
  ...
  struct Arm{const char*name;double dose;bool constraint;};std::vector<Arm>arms={{"0",0,false},{"r1",1,false}};
