...
SOLUTIONS = pathlib.Path("/workspace/prism-wave6-d19/fixed-solutions")
...
spec = importlib.util.spec_from_file_location(
    "d19_score_core", ROOT / "research/eta2_research_20260912/separable_rescue/core.py")
...
def summarize(cases: list[dict]) -> dict:
    ...
    for index in range(5):
        base, row = by[index]["0"], by[index]["r1"]
    ...
    check["passes"] = (gate_ok and check["ratio_pass_count"] >= 4
                       and check["decrease_pass_count"] >= 4
                       and check["all_healthy_retained"]
                       and check["all_rho_nondecreasing"])
    compact = []
    for index in range(5):
        for arm in ARMS:
            row = by[index][arm]
    ...
    return {
        ...
        "rank_one_check": check,
        "native_arm_earned": check["passes"],
        "decision": ("advance rank-one prior to native gate" if check["passes"]
                     else "stop rank-one prior at fixed-state gate"),
...
def main() -> None:
    ...
    provenance = {
        "protocol_sha256": sha(ROOT / "research/eta2_wave6/D19_RANK_ONE_SLOPPY_PRIOR_PROTOCOL.md"),
        "d18_capture_manifest_sha256": sha(ROOT / "research/eta2_wave6/d18_geometric_prior/capture-manifest.json"),
        "build_manifest_sha256": sha(HERE / "build-manifest.json"),
        "fixed_run_manifest_sha256": sha(HERE / "fixed-run-manifest.json"),
    }
    (ROOT / "research/eta2_wave6/d19-rank-one-prior-results.json").write_text(
        json.dumps({"provenance": provenance, "cases": cases}, indent=2) + "\n")
    (ROOT / "research/eta2_wave6/d19-rank-one-prior-summary.json").write_text(
