...
def execute(reg):
    ...
    for rep in range(reg["compatibility"]["repetitions"]):
        ...
        if rep % 2:
            arms.reverse()
        for arm in arms:
            rows.append(run_one(reg, "d0v3-compatibility", cell, arm, rep))
    ...
    for name, cell in reg["repeatability"]["cells"].items():
        for rep in range(reg["repeatability"]["repetitions"]):
            rows.append(run_one(reg, "d0v3-repeatability", cell, "deterministic", rep))
            N.write(P / "d0v3-results.json", rows)
    return rows
...
def summarize(reg, rows):
    ...
    N.write(P / "d0v3-summary.json", summary)
    return summary
