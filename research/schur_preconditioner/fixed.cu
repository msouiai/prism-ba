...
int main(int argc,char**argv){if(argc!=2)return 2;std::string dir=std::string(argv[1])+"/";int cd,nc,np,no;double sig,rad,eta;FILE*f=fopen((dir+"dimensions.txt").c_str(),"r");if(!f||fscanf(f,"%d %d %d %d %lf %lf %lf",&cd,&nc,&np,&no,&sig,&rad,&eta)!=7)return 2;fclose(f);if(cd!=9||nc<=0||np<=0||no<=0)return 2;int n=9*nc,grid=(n+255)/256;
