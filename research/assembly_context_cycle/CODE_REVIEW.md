...
Both attributed three-outer runs had three accepts, zero rejects, and three completed phase chains, so retry factor work is not omitted. Muell reports 139.120 ms in whole `MFAssemble`, 8.391 ms in point factor/RHS, and 5.453 ms in reduced-RHS work. Final13682 reports 884.435 ms, 137.418 ms, and 96.724 ms respectively. The assembly kernel is the dominant measured preparation phase. These are sums over three calls, not per-call values.

The labels follow event boundaries rather than source-level arithmetic. In particular, the native `W5ReducedRhsEquil` launch combines reduced-RHS formation and scaling; its time cannot be divided between those mathematical operations. `RobustUpdateScale` lies in the interval leading into assembly but is a no-op for the registered L2 configuration. Both endpoint audits agree with native output within roughly FP64 rounding scale. Profile/control trajectories remain unordered and should be compared tolerantly; these runs are not a fixed-order compatibility gate.
