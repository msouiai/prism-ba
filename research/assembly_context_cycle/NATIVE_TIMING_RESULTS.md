...
The other large recorded preparation boundaries were 9.446/137.402 ms for point factor plus point RHS and 5.922/96.690 ms for the camera Schur correction on Muell/Final13682. Equilibration follows in its own 0.029/0.107 ms interval. The covered-chain percentage is not total GPU utilization; candidate evaluation, Krylov work, and other solve work lie outside it.
...
The first run used the same binary and numerical configuration, but its evidence directory was overwritten when setup-field extraction was amended. Its recorded summary is therefore preliminary only and is preserved in `MATH_REVIEW.md`; no raw rows from that cohort remain and it is excluded from timing claims. All retained evidence is under `evidence/native-timing-v2` and is bound by `NATIVE_TIMING_RESULTS_V2.json`.

Whole-kernel timing establishes that assembly is the largest recorded assembly/preparation interval on both scenes. It cannot distinguish projection derivatives, compact writes, and camera/point accumulation because those operations share one monolithic kernel. Candidate selection remains pending the separately validated, non-additive split replay.
