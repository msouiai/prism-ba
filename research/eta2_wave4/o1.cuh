...
struct Solver {
 ...
 void assembleBlocks(){
  U.zero();V.zero();bc.zero();bp.zero();assemble<<<grid(p.nobs),256>>>(p,s,v.p,w.p,U.p,V.p,W.p,bc.p,bp.p);resetBad();inverse<<<grid(p.ncam),256>>>(p.ncam,U.p,ui.p,bad);inverse<<<grid(p.npt),256>>>(p.npt,V.p,vi.p,bad);checkBad();
