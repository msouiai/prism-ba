// Probe-only, unrounded camera/point cross blocks. Ordinary fragments survive.
__global__ void DepthCross64(const int* ci,const int* pi,const double* uv,
    const double* R,const double* t,const double* X,const double* f,
    const double* k1,const double* k2,const int* slots,int no,double k2mask,double* W){
  ...
  for(int i=0;i<9;++i)for(int j=0;j<3;++j)
    W[(size_t)(3*i+j)*no+slots[o]]=gx[i]*gx[9+j]+gy[i]*gy[9+j];
}
