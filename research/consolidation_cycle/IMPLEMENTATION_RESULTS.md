...
The owned workspace derivative passes exact serial compatibility and a same-process, same-GPU two-thread smoke on distinct tiny inputs and capacities. It owns active BAL cost, deterministic reduction, and diagnostic scratch per solve/device, restores caller devices, and handles injected partial construction failure. The smoke does not prove execution overlap or exercise every alternate path. Rig and whole-solver exception cleanup remain outside the proven scope.
