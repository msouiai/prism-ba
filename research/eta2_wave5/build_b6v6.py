#!/usr/bin/env python3
"""Build deterministic camera-major RHS on top of the B6v4 overlay."""
...
def load_b6v4():
    spec = importlib.util.spec_from_file_location("build_b6v4", P / "build_b6v4.py")
...
def derive():
  ...
  if(w5_prep_fuse){''',
    ...
    std::printf("W5_PREP_FUSE active ncam=%d npt=%d nobs=%d\\n",ncam,npt,nobs);
        ...
        '''      throw std::runtime_error("B6v4 prep fusion requires frozen champion-like CD9 classical diagonal path");
    if(w5_prep_camera && compact_mode!=2)
      throw std::runtime_error("B6v6 camera RHS requires camera-major compact fragments mode 2");
    ...
    patch(
        ...
        '''      if(w5_prep_fuse){
        if(w5_prep_camera)
          MFRhsDiagCamera<CD,Fragment><<<ncam,256>>>(Gc?Gc:Gp,p.mf_cspt,p.mf_coff,Rf,uu,nobs,corr,nullptr,fragment_slots);
        else
          MFRhsPrime<CD,Fragment><<<GridSize(nobs),256>>>(Gp,fragment_cams,fragment_points,uu,nobs,corr);
        ++w5_prep_calls;
      } else {''',
    )
    # B6v4 zeroes corr before the branch.  Camera ownership writes every entry,
    # so keep that harmless memset in this first registered implementation; it
    # isolates reduction order from another launch-level change.
    patch(
        '''  if(w5_prep_fuse)std::printf("W5_PREP_FUSE summary calls=%ld\\n",w5_prep_calls);''',
        '''  if(w5_prep_fuse)std::printf("W5_PREP_FUSE summary calls=%ld camera_rhs=%d\\n",w5_prep_calls,(int)w5_prep_camera);''',
    )
...
def main():
    ...
    with (build / "b6v6-build.log").open("w") as log:
    ...
    manifest = {
        ...
        "protocol_sha256": sha(P / "B6V6_PROTOCOL.md"),
    }
    (P / "b6v6-build-manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
    print("BUILT B6V6", manifest["binary_sha256"])
