...
def main():
 phase=sys.argv[1];assert phase=='joint'
 ...
 protocol=dict(phase=phase,jobs=jobs,flags=flags,binary_sha256=sha(ROOT/'prism'),source_sha256=sha(ROOT/'source.cu'),data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},cap=4,policy='Exact 2D joint camera/point TR subproblem in block-diagonal GN metric. Current radius then selective half/below-norm or double. True nonlinear acceptance and full GN verification. Incumbent direction retained if feasible and better. Radius persists across accepted states. Original CG camera controller supplies seeds; no new Krylov basis. Fixed before runs, N3 cap4s two scenes.',headers={str(p):sha(p) for p in (ROOT/'tooling').iterdir()})
 ...
 for j in jobs:
  ...
  env=dict(base,**flags,OCA_JOINT_TR=str(j['mode']),OCA_TARGET_COST=str(j['target']),OCA_MAX_SECONDS='4')
  if j['trace']:env['OCA_JOINT_TRACE']='1'
