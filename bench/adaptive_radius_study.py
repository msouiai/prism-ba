...
def main():
 phase=sys.argv[1];assert phase in ['adaptive','ray']
 mode=1 if phase=='adaptive' else 2;out=ROOT/phase;out.mkdir(exist_ok=True)
 ...
 protocol=dict(phase=phase,jobs=jobs,flags=flags,binary_sha256=sha(ROOT/'prism'),source_sha256=sha(ROOT/'source.cu'),data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},cap=4,policy='Mode1: projected solve depth16/32/64/128 verified relative residual<=.1; CG fallback on cap failure. Selective double radius after reliable boundary-limited step; shrink below rejected norm. Mode2: existing bank, exact quadratic minimization along each saved ray for neighboring radius. Same selective nonlinear test. Both retain point safeguards. No learn log or profiler in target timings.',headers={str(p):sha(p) for p in (ROOT/'tooling').iterdir()})
