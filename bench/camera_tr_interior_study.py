#!/usr/bin/env python3
"""Revised matched-width TR ablation with shared interior-step damping relaxation."""
...
def main():
 ...
 binaries={arm:str(ROOT/'prism-tr') for arm in ['tr1','tr5']}
 jobs=[dict(name=s+'-'+arm+'-sanity',scene=s,target=1e-100,arm=arm,cap=4,iterations=6,rep=0,sanity=True) for s,_ in SCENES for arm in ['tr1','tr5']]
 for rep in range(3):
  for i,(scene,target) in enumerate(SCENES if rep%2==0 else SCENES[::-1]):
   arms=['tr1','tr5'] if rep%2 else ['tr5','tr1']
   for arm in arms:jobs.append(dict(name=f'{scene}-{arm}-{rep+1}',scene=scene,target=target,arm=arm,cap=4,iterations=100000,rep=rep+1,sanity=False))
 source=pathlib.Path(__file__).resolve().parent
 files=[source/'camera_tr_interior_study.py',source/'build_camera_tr_interior.py',source/'audit_prism_state.py',source/'cached_benchmark_input.py',source/'profile_iterations.py',source/'novelty_ablation.py']
 ...
 protocol=dict(flags=flags,jobs=jobs,binaries=binaries,binary_sha256={a:sha(b) for a,b in binaries.items()},source_sha256=sha(ROOT/'source-tr.cu'),data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},
  ...
  budget='Four six-iteration sanity runs then 12 target runs; all cap4s. Same binary and controller, only OCA_NSHIFTS differs. Arm order reverses in repeat two. One shared interior damping correction was fixed after phase-one diagnosis, before these runs; no parameter sweep or further tuning.')
 ...
 for j in jobs:
  ...
  env=dict(base,**flags,OCA_NSHIFTS='1' if j['arm']=='tr1' else '5',OCA_TARGET_COST=str(j['target']),OCA_MAX_SECONDS=str(j['cap']))
  if j['arm'] in ['tr1','tr5']:env['OCA_CAMERA_TR']='1'
  ...
  if j['arm'] in ['tr1','tr5']:assert tr and extra
