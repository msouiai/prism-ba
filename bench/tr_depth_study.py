...
ROOT=pathlib.Path('/workspace/prism-joint-tr');OUT=ROOT/'depth64'
...
def main():
 ...
 arms=[('tr128',0,'8,16,32,64,128'),('tr64',0,'8,16,32,64'),('joint64',1,'8,16,32,64')]
 ...
 protocol=dict(jobs=jobs,flags=flags,binary_sha256=sha(ROOT/'prism'),source_sha256=sha(ROOT/'source.cu'),headers={str(p):sha(p) for p in (ROOT/'tooling').iterdir()},data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},cap=4,reason='Full-depth joint variant lost; traces show late solves often at128. Test one halving of linear budget with ordinary TR control to isolate subspace geometry. No other parameter change. Cauchy candidate retained. N3, fixed targets, no profiling or detail traces.')
