...
ARMS={'legacy':{'OCA_DEMAND_MENU':'2'},'checked':{'OCA_DEMAND_MENU':'2','OCA_KRYLOV_REUSE':'3','OCA_CAPTURE_VERIFY_PREFIX':'1'},'fast':{'OCA_DEMAND_MENU':'2','OCA_KRYLOV_REUSE':'3'}}

def main():
 p=argparse.ArgumentParser();p.add_argument('--root',type=pathlib.Path,default=pathlib.Path('/workspace/prism-recycle-cg-fast'));a=p.parse_args();root=a.root;out=root/'runs';out.mkdir(exist_ok=True);binary=root/'prism-frozen';targets=json.loads((root/'targets.json').read_text())
