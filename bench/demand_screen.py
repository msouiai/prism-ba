#!/usr/bin/env python3
"""Frozen, short CPU-audited demand-menu ablation; no Caspar reruns."""
...
ARMS={'single':{'OCA_NSHIFTS':'1'},'multi':{},'demand':{'OCA_DEMAND_MENU':'1'},'pair':{'OCA_DEMAND_MENU':'2'}}
SCENES={'ladybug-1197':3,'venice-52':3,'dubrovnik-356':8}
def main():
 p=argparse.ArgumentParser();p.add_argument('--root',type=pathlib.Path,default=pathlib.Path('/workspace/prism-demand'));a=p.parse_args();root=a.root;out=root/'screen';out.mkdir(exist_ok=True);binary=root/'prism-frozen'
 ...
 for si,(scene,budget) in enumerate(SCENES.items()):
  ...
  for rep in [1,2]:
   order=list(ARMS);j=(si+rep-1)%4;order=order[j:]+order[:j]
   ...
   for arm in order:
    ...
    cmd=['flock','/tmp/prism_gpu.lock','timeout','90',str(binary),'--problem',str(data),'--algo','mfree_shifted_cg','--dof9','--zero_k2','--max_iter','100000','--csv',str(stem.with_suffix('.csv')),'--state_out',str(stem.with_suffix('.state'))]
