#!/usr/bin/env python3
"""Bounded CG-stopping experiment; historical arm labels double=control, storage=candidate."""
...
BINS={'double':pathlib.Path('/workspace/prism-tr-mixed/storage/prism-tr'),'storage':ROOT/'build/prism-tr'}
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--phase',choices=['audit','timing'],required=True);ap.add_argument('--scenes',nargs='+',default=list(SCENES));ap.add_argument('--reps',type=int,default=3);ap.add_argument('--output',type=pathlib.Path);ap.add_argument('--candidate',type=pathlib.Path);ap.add_argument('--audit-full',action='store_true');ap.add_argument('--trace',action='store_true');ap.add_argument('--mode',default='1');a=ap.parse_args()
 ...
 protocol=dict(jobs=jobs,flags=flags,bins={k:sha(v) for k,v in BINS.items()},scenes={s:dict(nominal=SCENES[s][0],cap=SCENES[s][1],input_sha256=sha('/workspace/bal/'+s+'.txt')) for s in a.scenes},scope=globals().get('SCOPE_OVERRIDE','Control=frozen mixed-buffer TR; storage arm=CG stopping candidate. FP64 calculations/state/model acceptance. Fixed 5% FW gap rule, conditional on convexity, not certified for rounded Schur operator. Audit runs use 7 iterations and extra curvature matvecs; timing runs have no instrumentation. Native crossing certified only by independent original-double endpoint. Capped misses retained; solver-boundary overshoot charged.'))
 protocol['candidate_manifest']=json.loads((BINS['storage'].parent/'stop-manifest.json').read_text());protocol['stop_mode']=a.mode;protocol['trace']=a.trace
 ...
 for j in jobs:
  ...
  nominal,cap=SCENES[scene];env=dict(base,**flags,OCA_TARGET_COST=str(nominal),OCA_MAX_SECONDS=str(cap));
  if arm=='storage':env['OCA_CG_STOP']=a.mode
  if a.trace:env['OCA_CG_STOP_TRACE']='1'
  cmd=['flock','/tmp/prism_gpu.lock','timeout','240',str(BINS[arm]),'--problem','/workspace/bal/'+scene+'.txt','--algo','mfree_shifted_cg','--dof9','--zero_k2','--max_iter','7' if a.phase=='audit' and not a.audit_full else '100000','--csv',str(stem)+'.csv','--state_out',str(stem)+'.state']
 ...
 for scene in a.scenes:
  ...
  for arm in BINS:
   ...
   d[arm]=dict(hits=len(hits),runs=len(rr),median_crossing=statistics.median(hits) if len(hits)==len(rr) else None,median_endpoint=statistics.median(r['cost'] for r in rr if 'cost' in r) if any('cost' in r for r in rr) else None)
  summary.append(d)
