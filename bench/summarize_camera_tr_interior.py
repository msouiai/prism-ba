...
from camera_tr_interior_study import ROOT,SCENES
def main():
 ...
 assert sha(ROOT/'source-before.cu')==sha('/workspace/prism-camera-tr-ablation/source-tr.cu')
 ...
 replacement="""// A feasible interior step with an adequate model must not leave
 ...
 expected=before.replace(old,replacement)
 ...
 for r,j in zip(rows,p['jobs']):
  ...
  accepted=[d for d in r['tr_trace'] if d['accept']]
  assert len(accepted) in [len(r['lambda_trace']),len(r['lambda_trace'])+1]
  previous=10.;r['verified_interior_updates']=0
  for d,next_lambda in zip(accepted,r['lambda_trace']):
   if d['norm']<.8*d['radius'] and d['rho']>=.25:
    assert next_lambda<=max(1e-7,.1*previous)*1.003
    r['verified_interior_updates']+=1
   previous=next_lambda
