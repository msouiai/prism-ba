...
ROOT=pathlib.Path('/workspace/prism-early-radius')
SCENES=[('trafalgar-126',104534.24152926281),('dubrovnik-88',359003.9111293723)]
def main():
 phase=sys.argv[1];assert phase=='early'
 ...
 protocol=dict(phase=phase,jobs=jobs,flags=flags,binary_sha256=sha(ROOT/'prism'),source_sha256=sha(ROOT/'source.cu'),data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},cap=4,policy='Mode2: only at outer0, test neighboring radius with exact quadratic ray minimization on the existing bank and full GN acceptance. All subsequent iterations use unchanged corrected TR-one. No projected basis. Fixed after adaptive and persistent ray variants lost. Both arms same binary, N3,cap4s, frozen targets, logs and profiler disabled in timings.',headers={str(p):sha(p) for p in (ROOT/'tooling').iterdir()})
