#!/usr/bin/env python3
"""Render the measured storage-precision study without imputing censored times."""
import pathlib,json,statistics
ROOT=pathlib.Path('/workspace/prism-tr-mixed');REPO=pathlib.Path(__file__).resolve().parents[1]
def main():
 verification=json.loads((ROOT/'verification.json').read_text());primary=json.loads((ROOT/'timing/summary.json').read_text());points=json.loads((ROOT/'point-double-timing/summary.json').read_text());extra=json.loads((ROOT/'extra-scenes/results.json').read_text());profile=json.loads((ROOT/'profile-summary.json').read_text())
 lines=['# TR mixed-storage investigation — 2026-09-09','',
 ...
 for r in primary:
  a,b=r['double'],r['storage'];av,bv=a['median_crossing'],b['median_crossing']
  lines.append(f'| {r["scene"]} | '+(f'{av:.3f}s' if av else '0/3 hits, 20s cap')+' | '+(f'{bv:.3f}s' if bv else '0/3 hits, 20s cap')+' | '+(f'{bv/av:.3f}×' if av and bv else '—')+' |')
 lines += ['',
 ...
 for scene in ['ladybug-1197','final-4585']:
  cells=[]
  for arm in ['tr','caspar64','caspar32']:
   r=next(r for r in extra if r['scene']==scene and r['arm']==arm)
   cells.append(f'{r["crossing"]:.3f}s' if r['hit'] else f'Miss ({r["cap"]}s cap)')
 ...
 for scene in ['final-1936','final-13682']:
  p=profile[scene]['tr_profile'];lines.append('| '+scene+' | '+' | '.join(f'{p[k]:.3f}s' for k in ['assembly','pointfactor_rhs','krylov','candidates','runtime'])+' |')
 lines += ['',
 ...
 '```text', 'ΔS = −ΔW Vτ⁻¹ Wᵀ − W Vτ⁻¹ ΔWᵀ − ΔW Vτ⁻¹ ΔWᵀ.', '```', '',
 ...
 for r in points:lines.append(f'| {r["scene"]} | {r["double"]["median_crossing"]:.3f}s | {r["storage"]["median_crossing"]:.3f}s |')
 lines += ['',
 ...
 f'All {verification["endpoints"]} exported endpoints were independently re-audited: 58 experiment endpoints and five profiling endpoints. Maximum agreement error against the respective independent/native checker is {verification["max_audit_error"]:.3g}. There were {verification["accepted_tr_checks"]} accepted TR feasibility checks, {verification["rho_identity_checks"]} checks of actual/predicted reduction (maximum relative discrepancy {verification["max_rho_identity_error"]:.3g}), and {verification["curvature_checks"]} explicit-versus-recurrence curvature checks (maximum discrepancy {verification["max_curvature_error"]:.3g}, threshold 1e-7). No process failures. Target misses remain misses.',
 '', f'The 58 experiment runs consumed {verification["native_experiment_seconds"]:.3f}s of native solver time, excluding the separately labelled profiles, input processing, auditing and any GPU-lock wait. GPU runs were serialized. All 11 pre-existing paused jobs remained paused. Process wall times include lock waits and unequal input/auditing scopes and are not comparable application latency.',
 ...
 (REPO/'docs/tr_mixed_storage_results.md').write_text('\n'.join(lines))
 print(REPO/'docs/tr_mixed_storage_results.md')
