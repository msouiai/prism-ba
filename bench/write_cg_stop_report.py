...
ROOT=pathlib.Path('/workspace/prism-tr-cg-stop');REPO=pathlib.Path(__file__).resolve().parents[1]
def main():
 ver=json.loads((ROOT/'verification.json').read_text());small=json.loads((ROOT/'guarded-timing/summary.json').read_text());fw=json.loads((ROOT/'fw/summary.json').read_text());large=json.loads((ROOT/'large/results.json').read_text())
 lines=['# CG stopping and projected TR — 2026-09-09','',
 ...
 labels={'tr':'Guarded projected TR','control':'Frozen mixed-buffer TR','caspar64':'Caspar FP64','caspar32':'Caspar FP32, stopping margin'}
 for r in large:lines.append(f'| {labels[r["arm"]]} | '+(f'{r["crossing"]:.3f}s' if r['hit'] else 'Miss')+f' | {r["cost"]:,.3f} |')
 lines += ['',
 ...
 for r in small:
  a=r['double']['median_crossing'];b=r['storage']['median_crossing'];lines.append(f'| {r["scene"]} | {a:.3f}s | {b:.3f}s | {100*(b/a-1):+.1f}% |')
 lines += ['',
 ...
 for line in (ROOT/'replay-basis.log').read_text().splitlines():
  if line.startswith('BASIS_TR '):
   v={k:float(v) for k,v in re.findall(r'(\w+)=([^ ]+)',line)};lines.append(f'| {int(v["depth"])} | {v["prediction"]:,.1f} | {v["ratio"]:.4f} | {v["lambda"]:.6g} |')
 lines += ['',
 ...
 '', '```text', 'A = S + sigma I', 'p_j = r_j + beta_(j−1) p_(j−1)', 'S r_j = A p_j − beta_(j−1) A p_(j−1) − sigma r_j.', '```',
 ...
 for r in fw:lines.append(f'| {r["scene"]} | {r["double"]["median_crossing"]:.3f}s | {r["storage"]["median_crossing"]:.3f}s |')
 lines += ['',
 ...
 f'{ver["total_ba_endpoints"]} completed BA endpoints were independently audited, including the fixed-system capture and the instrumented largest-scene repeat. Maximum endpoint agreement error is {ver["max_endpoint_error"]:.3g}. The earlier failed prototype has no exported endpoint and is not counted as a passing audit. There are {ver["accepted_checks"]} checked accepted TR trace records, {ver["curvature_checks"]} recurrence-curvature checks (maximum {ver["max_curvature_error"]:.3g}), and {ver["projected_checks"]} projected-model checks from completed diagnostic runs (maximum {ver["max_projected_error"]:.3g}). Four frozen-operator replay checks agree within {ver["max_replay_error"]:.3g} of the RHS norm.',
 ...
 (REPO/'docs/cg_stopping_results.md').write_text('\n'.join(lines));print(REPO/'docs/cg_stopping_results.md')
