...
R=pathlib.Path('/workspace/prism-tr-novelty-ablation');BIN=R/'build/prism-tr';FROZEN=pathlib.Path('/workspace/prism-tr-preconditioner/pcg-v2/prism-tr')
...
def main():
 ...
 protocol=dict(jobs=jobs,flags={arm:flags(arm) for arm in a.arms},binary_sha256=sha(a.binary),frozen_sha256=sha(FROZEN),inputs={s:sha('/workspace/bal/'+s+'.txt') for s in a.scenes},iterations=a.iterations,scope='Same optimized kernels, FP64 arithmetic/state/model acceptance with FP32 fragment storage, Hcc block PCG. TR-no-safe removes only per-point safeguard. TR-no-projection removes projected z-basis and its FW stopping path but retains legacy TR candidate bank/controller/safeguard. Classical LM uses coupled lambda damping, Hcc diagonal camera scaling, Nielsen updates, terminal PCG step only, no line search or per-point safeguard. Numerical point damping floors retained; fused diagonal work retained even where unused. Initial lambda10 except lm_low=1e-4 sensitivity arm, EW maximum.5 and PCG checkpoint cap. Solver-native capped time to nominal*(1-1e-8), independently audited on original-double observations. Misses retained. No Caspar runs in this internal ablation. Optional lm_safe/lm_refine suffixes use isolated failed-step point rescue 1/2; all _low arms start lambda1e-4.')
 ...
 for j in jobs:
  ...
  if p.returncode==0:
   ...
   for line in log.splitlines():
    if line.startswith(('CAMERA_TR o=','CLASSICAL_LM o=')):
     ...
     if line.startswith('CLASSICAL_LM'):
      assert v['tau']==v['lambda'];lm.append(v)
   for line in log.splitlines():
   ...
   if arm.startswith('lm'):
    assert lm and 'CAMERA_TR o=' not in log and 'POINT_SAFE summary' not in log
    nu=2
    for prev,nxt in zip(lm,lm[1:]):
