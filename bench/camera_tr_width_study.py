#!/usr/bin/env python3
"""Matched one/five-shift TR ablation; identical controller and Cauchy candidate."""
...
def main():
 ...
 files=[source/'camera_tr_width_study.py',source/'build_camera_tr_ablation.py',source/'audit_prism_state.py',source/'cached_benchmark_input.py',source/'profile_iterations.py',source/'novelty_ablation.py']
 ...
 protocol=dict(flags=flags,jobs=jobs,binaries=binaries,binary_sha256={a:sha(b) for a,b in binaries.items()},source_sha256=sha(ROOT/'source-tr.cu'),data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},
  ...
  budget='Four six-iteration sanity runs then 12 target runs; all cap4s. Same binary and controller, only OCA_NSHIFTS differs. Arm order reverses in repeat two; no post-outcome tuning.')
