...
def main():
 p=argparse.ArgumentParser();p.add_argument('--root',type=pathlib.Path,default=pathlib.Path('/workspace/prism-equal-quality'));a=p.parse_args();root=a.root;out=root/'runs';out.mkdir(exist_ok=True);binary=root/'prism-frozen';targets=json.loads((root/'targets.json').read_text())
 ...
 plan=dict(arms=ARMS,targets=targets,reps=3,binary_sha256=sha(binary),flags={k:v for k,v in base.items() if k.startswith('OCA_')})
 pp=root/'plan.json'
