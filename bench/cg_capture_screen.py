...
ARMS={'legacy':{'OCA_DEMAND_MENU':'2'},'rebuild':{'OCA_DEMAND_MENU':'2','OCA_KRYLOV_REUSE':'4'},'reuse':{'OCA_DEMAND_MENU':'2','OCA_KRYLOV_REUSE':'3'}}
SCENES={'ladybug-1197':3,'dubrovnik-356':8}
def main():
 global ARMS,SCENES
 p=argparse.ArgumentParser();p.add_argument('--root',type=pathlib.Path,default=pathlib.Path('/workspace/prism-recycle-cg'));p.add_argument('--fixed-work',action='store_true');a=p.parse_args();root=a.root;
 ...
 for si,(scene,budget) in enumerate(SCENES.items()):
  ...
  for rep in [1,2]:
   ...
   for arm in order:
    ...
    if r.returncode==0:
     records=[dict(zip(['outer','reused','captured','imported','depth','residual','tolerance','fallback'],map(float,m))) for m in re.findall(r'CAPTURE o=(\d+) reused=(\d+) captured=(\d+) imported=(\d+) depth=(\d+) residual=([\deE.+-]+|inf) tolerance=([\deE.+-]+) fallback=(\d+)',log)]
     assert len(records)==log.count('CAPTURE o=')
     assert all(t['residual']<=t['tolerance'] for t in records if not t['fallback'])
     row.update(capture_attempts=len(records),reuse_hits=sum(t['reused'] for t in records),fallbacks=sum(t['fallback'] for t in records),retained_vectors=sum(t['imported'] for t in records if t['reused']))
