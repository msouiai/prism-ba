...
#include "solver.h"  // f32 build: gen32 on include path
...
int main(int argc, char** argv) {
  ...
  std::vector<float> sfr(7 * (size_t)nobs, 0.0f);
  ...
  std::vector<float> ppc(2 * (size_t)nobs, 0.0f);  // pp constant (0,0) per factor
