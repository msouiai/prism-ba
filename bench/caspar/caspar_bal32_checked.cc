...
// Float state and factors; independent CPU costs use original double pixels.
...
int main(int argc, char** argv) {
  ...
  caspar::SolverParams<float> params;
  ...
  if(const char* e=std::getenv("CASPAR_TARGET_COST")) {
    ...
    params.score_exit_value=static_cast<float>(target);
  }
  ...
  std::vector<float> fae(2 * (size_t)ncam);
  ...
  std::vector<float> sfr(7 * (size_t)nobs, 0.0);
  ...
  std::vector<float> ppc(2 * (size_t)nobs, 0.0);  // pp constant (0,0) per factor
  ...
  for (int i = 0; i < nobs; ++i) {
    const float* q = pose.data() + 7 * ocam[i];
    const float* p = pts.data() + 3 * opt[i];
  ...
  std::printf("CHECK_INITIAL score=%.17g precision=f32\n", CheckCost());
