#!/usr/bin/env python3
"""Audit and summarize the fixed camera-space trust-region screen."""
import json,pathlib,re,statistics
from expanded_caspar_screen import sha,write
from camera_tr_study import ROOT,SCENES
def main():
 p=json.loads((ROOT/'protocol.json').read_text());rows=json.loads((ROOT/'results.json').read_text())
 assert len(rows)==20 and [r['name'] for r in rows]==[j['name'] for j in p['jobs']]
 ...
 assert sha(ROOT/'source-before.cu')==sha(pathlib.Path(__file__).parents[1]/'gpu/oca_cuda.cu')
 ...
 for r,j in zip(rows,p['jobs']):
  ...
  flags=dict(p['flags'],OCA_NSHIFTS='1' if j['arm']=='single' else '5',OCA_TARGET_COST=str(j['target']),OCA_MAX_SECONDS=str(j['cap']))
  if j['arm']=='tr':flags['OCA_CAMERA_TR']='1'
