...
def main():
 phase=sys.argv[1];assert phase=='recurrence'
 ...
 jobs=[]
 for s,t in SCENES:
  for w in [1,5]:jobs.append(dict(scene=s,target=1e-100,mode=mode,rep=-w,iterations=16,trace=True,width=w))
 ...
 protocol=dict(phase=phase,jobs=jobs,flags=flags,binary_sha256=sha(ROOT/'prism'),source_sha256=sha(ROOT/'source.cu'),data_sha256={s:sha(pathlib.Path('/workspace/bal')/(s+'.txt')) for s,_ in SCENES},cap=4,policy='Replace explicit Schur curvature matvec at each CG snapshot with bdot-sigma*norm2-zeta*x_dot_recursive_residual. Full nonlinear cost and full GN acceptance unchanged. Four16-iteration audit runs cover widths1/5 on both scenes with explicit operator comparisons threshold1e-7. Timings compare original vs recurrence TRone N3 same binary fixed targets4s.',headers={str(p):sha(p) for p in (ROOT/'tooling').iterdir()})
 ...
 for j in jobs:
  ...
  env=dict(base,**flags,OCA_TR_RECURRENCE=str(j['mode']),OCA_TARGET_COST=str(j['target']),OCA_MAX_SECONDS='4')
