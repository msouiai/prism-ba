#!/usr/bin/env python3
"""Bounded largest-scene CG projection comparison, N1 four arms."""
...
ROOT=pathlib.Path('/workspace/prism-tr-cg-stop/large')
BINS={'tr':pathlib.Path('/workspace/prism-tr-cg-stop/guarded/prism-tr'),'control':pathlib.Path('/workspace/prism-tr-mixed/storage/prism-tr'),'caspar64':pathlib.Path('/workspace/prism-caspar-current/caspar64'),'caspar32':pathlib.Path('/workspace/prism-caspar-current/caspar32')}
SCENES=[('final-13682',27318392.631312046,20)]
def main():
 ...
 protocol=dict(scenes=SCENES,flags=flags,bins={k:sha(v) for k,v in BINS.items()},inputs={s:sha('/workspace/bal/'+s+'.txt') for s,_,_ in SCENES},scope=globals().get('SCOPE_OVERRIDE','N1 largest-scene screen: guarded CG-projected TR, frozen mixed-buffer control, Caspar64 and Caspar32. Float fragments, double calculations/state/acceptance. Effective audited target nominal*(1-1e-8). Caspar32 uses preregistered .1% tighter native stopping margin (float-rounded); its time is crossing that stricter stop, not an earlier uncertified trace point. Independent original-double endpoint required. No instrumentation; misses not speed ratios.'))
 ...
 for i,(scene,nominal,cap) in enumerate(SCENES):
  ...
  for arm in arms:
   ...
   if arm in globals().get('PRISM_ARMS',('tr','control')):
    env.update(flags,OCA_TARGET_COST=str(nominal),OCA_MAX_SECONDS=str(cap))
    if arm=='tr':env['OCA_CG_STOP']='2'
    cmd=[str(BINS[arm]),'--problem','/workspace/bal/'+scene+'.txt','--algo','mfree_shifted_cg','--dof9','--zero_k2','--max_iter','100000','--state_out',str(stem)+'.state']
   else:
    env.update(CASPAR_TARGET_COST=str(target*.999 if arm=='caspar32' else target),CASPAR_MAX_SECONDS=str(cap),CASPAR_STATE_OUT=str(stem)+'.state');cmd=[str(BINS[arm]),'/workspace/bal/'+scene+'.txt','100000','default']
   ...
   if q.returncode==0:
    ...
    if arm in globals().get('PRISM_ARMS',('tr','control')):
     ...
     reported=float(re.search(r'CHECK final_score=(\S+)',log)[1]);seconds=float(re.search(r'RESULT .*?runtime=(\S+)',log)[1]);native_target=target
     if arm=='caspar32':
      import struct
      native_target=struct.unpack('f',struct.pack('f',target*.999))[0]
     trace=re.findall(r'TRACE iter=\d+ cost=(\S+) seconds=(\S+)',log);cross=min((float(t) for c,t in trace if float(c)<=native_target),default=None)
